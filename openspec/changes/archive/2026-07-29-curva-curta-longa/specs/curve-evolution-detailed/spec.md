...
When the evolution checkbox and 3D checkbox are both ON and the "Curva curta" radio is selected, the system SHALL render the detailed evolution data as a 3D surface instead of 2D lines.

#### Scenario: 3D checkbox enables 3D detailed rendering
- **WHEN** evolution checkbox is ON
- **AND** 3D checkbox is ON
- **AND** "Curva curta" radio is selected
